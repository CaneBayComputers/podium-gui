...
for arg in "$@"; do
    case $arg in
        --help)
            echo "Zeltro GUI macOS Installer"
            echo ""
...
CLI_INSTALLER_URL="https://raw.githubusercontent.com/CaneBayComputers/podium-cli/master/install-mac.sh"
...
if [[ "$(uname -s)" != "Darwin" ]]; then
    echo -e "${RED}Error: This installer is for macOS${NC}"
    echo "Use install-ubuntu.sh, install-fedora.sh or install-arch.sh instead."
    exit 1
fi
...
if command -v zeltro >/dev/null 2>&1; then
    ...
    for _sibling in "$SELF_DIR/../zeltro-cli" "$CURRENT_DIR/../zeltro-cli"; do
        if [[ -n "$_sibling" && -f "$_sibling/install-mac.sh" ]]; then
    ...
    if [[ -n "$CLI_LOCAL" ]]; then
        ...
        ( cd "$CLI_LOCAL" && bash install-mac.sh )
    else
...
# Homebrew refuses to run under sudo, so this stays unprivileged. Xcode command
# line tools provide the compiler node-gyp needs for the native rebuild.
if ! xcode-select -p >/dev/null 2>&1; then
    echo -e "${BLUE}Installing Xcode command line tools...${NC}"
    xcode-select --install || true
    echo -e "${YELLOW}Finish the Xcode tools install if prompted, then re-run this script.${NC}"
fi
...
if [[ "$NODE_OK" -eq 0 ]]; then
    echo -e "${BLUE}Installing Node.js ${NODE_MIN_MAJOR}...${NC}"
...
###############################
# Step 7: application shortcut
###############################
# macOS has no .desktop files or hicolor icon theme, so the launcher on PATH is
# the install. A double-clickable .app bundle is a separate build:
#   npm run build-mac   (electron-builder, produces a .dmg)
echo -e "${CYAN}Skipping desktop entry (not applicable on macOS)${NC}"
...
echo
echo -e "${CYAN}📦 For a double-clickable app bundle:${NC}"
echo -e "  ${BLUE}cd $INSTALL_DIR && npm run build-mac${NC}"
echo
echo -e "${CYAN}🗑️  To uninstall:${NC}"
echo -e "  ${BLUE}sudo rm -rf $INSTALL_DIR $BIN_DIR/zeltro-gui${NC}"
echo
