...
if ! command -v pacman &> /dev/null; then
    ...
    echo "Use install-ubuntu.sh, install-fedora.sh or install-mac.sh instead."
    exit 1
fi
...
# without an initialized pacman keyring. Every install then dies with "required
# key missing from keyring". A normal desktop install already has this, so the
# probe skips it.
...
# Deliberately `-S --needed` rather than `-Syu`: a full system upgrade is the
# CLI installer's job, and doing it twice risks replacing the running kernel
# a second time (which stops Docker until reboot).
#
...
if [[ "$NODE_OK" -eq 0 ]]; then
    ...
    # Arch tracks current Node, which is always well past the minimum.
...
# which stops Docker from starting until the machine reboots.
if [ ! -d "/usr/lib/modules/$(uname -r)" ]; then
    echo -e "${YELLOW}⚠ The running kernel ($(uname -r)) was replaced by a system upgrade.${NC}"
    echo -e "${YELLOW}  Docker cannot start until you reboot (${BLUE}sudo reboot${YELLOW}).${NC}"
    echo
fi
