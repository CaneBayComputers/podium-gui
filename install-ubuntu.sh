...
# Podium GUI Installer — Ubuntu / Debian / Linux Mint
#
...
for arg in "$@"; do
    case $arg in
        --help)
            echo "Podium GUI Ubuntu Installer"
            echo ""
...
CLI_INSTALLER_URL="https://raw.githubusercontent.com/CaneBayComputers/podium-cli/master/install-ubuntu.sh"
...
if ! command -v apt-get &> /dev/null; then
    ...
    echo "Use install-fedora.sh, install-arch.sh or install-mac.sh instead."
    exit 1
fi
...
if command -v podium >/dev/null 2>&1; then
    ...
    for _sibling in "$SELF_DIR/../podium-cli" "$CURRENT_DIR/../podium-cli"; do
        if [[ -n "$_sibling" && -f "$_sibling/install-ubuntu.sh" ]]; then
    ...
    if [[ -n "$CLI_LOCAL" ]]; then
        ...
        ( cd "$CLI_LOCAL" && bash install-ubuntu.sh )
    else
