...
#
# The GUI is a front end for Zeltro CLI and is useless without it, so a package
...
if command -v zeltro >/dev/null 2>&1 \
   || [ -x /usr/local/bin/zeltro ] \
   || [ -x /usr/bin/zeltro ] \
   || [ -x /opt/zeltro-cli/src/zeltro ]; then
    exit 0
fi

echo "Zeltro CLI not found — installing it (the GUI requires it)."
...
sudo -u "$RUN_AS" bash -c "curl -fsSL '$CLI_INSTALLERS/$SCRIPT' | bash" || {
    echo "Zeltro CLI install did not complete. Run it manually:"
    echo "  curl -fsSL $CLI_INSTALLERS/$SCRIPT | bash"
}
exit 0
